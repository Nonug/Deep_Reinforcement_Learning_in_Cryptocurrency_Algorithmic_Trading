...
def testAI(network,name = "", animation = False, mode = 'test'):
    ...
    if mode == 'test':
        ...
        simulator.test(strategy, trainCryptocurrency, testCryptocurrency, AI_TRAINING_PARAM, AI_TRAINING_PARAM, suffix = name)
    else:
        simulator.test(strategy, trainCryptocurrency, testCryptocurrency, AI_TRAINING_PARAM, BACKTEST_PARAM, suffix = name)
    print()
