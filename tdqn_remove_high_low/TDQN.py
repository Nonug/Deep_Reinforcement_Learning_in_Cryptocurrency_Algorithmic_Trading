...
class TDQN:
    ...
    def processState(self, state, coefficients):
        ...
        volumes = [state[1][i] for i in range(len(state[1]))]
        s2f = [state[2][i] for i in range(len(state[2]))] # reduce state
        ...
        if coefficients[1][0] != coefficients[1][1]:
            state[1] = [((x - coefficients[1][0])/(coefficients[1][1] - coefficients[1][0])) for x in volumes]
        else:
            state[1] = [0 for x in volumes]
        # 5. s2f # reduce state
        s2f = [s2f[i] for i in range(1, len(s2f))]
        if coefficients[2][0] != coefficients[2][1]:
            state[2] = [((x - coefficients[2][0])/(coefficients[2][1] - coefficients[2][0])) for x in s2f]
        else:
            state[2] = [0 for x in s2f]
